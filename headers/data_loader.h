...
namespace simple_nn
{
	...
	public:
		DataLoader();
		DataLoader(MatXf& X, MatXf& Y_M, int batch, int channels,
					int height, int width, bool shuffle);
		DataLoader(MatXf& X, VecXi& Y, int batch, int channels,
					int height, int width, bool shuffle);
		void load (MatXf& X, MatXf& Y_M, int batch, int channels,
					int height, int width, bool shuffle);
		...
		MatXf get_y_matrix(int i) const;
	...
	void DataLoader::load(MatXf& X, MatXf& Y_M, int batch, int channels,
		int height, int width, bool shuffle)
	{
		this->X = std::move(X);
		this->Y_M = std::move(Y_M);
		this->batch = batch;
		ch = channels;
		h = height;
		w = width;
	...
	MatXf DataLoader::get_y_matrix(int i) const
	{
		unsigned int hw = 48 * 40;
		MatXf batch_y(batch, hw);
		for (int j = 0; j < batch; j++) {
			const float* first = Y_M.data() + batch_indices[i][j] * hw;
			const float* last = first + hw;
			float* dest = batch_y.data() + j * hw;
			std::copy(first, last, dest);
		}
		return batch_y;
	}
