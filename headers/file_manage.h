...
namespace simple_nn
{
	...
	MatXf read_spectral(string data_dir, string fname, int n_imgs, bool train = true)
	{
		MatXf img;
		...
		int n_rows = arr.shape[1];
		int n_cols = arr.shape[2];
		...
		for (int n = 0; n < n_imgs; n++) {
			for (int i = 0; i < n_rows; i++) {
				for (int j = 0; j < n_cols; j++) {
					img(n, n_cols * i + j) = loaded_data[(n_rows * n_cols * n)+
																(n_cols * i) + j];
				}
			}
		}
	...
	MatXf read_latent(string data_dir, string fname, int n_imgs, bool train = true)
	{
		MatXf img;
		...
		for (int n = 0; n < n_imgs; n++) {
			for (int i = 0; i < n_rows; i++) {
				for (int j = 0; j < n_cols; j++) {
					img(n * n_rows + i, j) = loaded_data[(n_rows * n_cols * n) +
															  (n_cols * i) + j];
				}
			}
		}
	...
	MatXf read_black_box(string data_dir, string fname, int n_imgs, bool train = true)
	{
		MatXf img;
		...
		cnpy::NpyArray arr = cnpy::npy_load(path);
		float* loaded_data = arr.data<float>();
		int n_chanels = arr.shape[1];
		int n_rows = arr.shape[2];
		int n_cols = arr.shape[3];

		img.resize(n_imgs * n_chanels, n_rows * n_cols);

		for (int n = 0; n < n_imgs; n++) {
			for (int k = 0; k < n_chanels; k++) {
				for (int i = 0; i < n_rows; i++) {
					for (int j = 0; j < n_cols; j++) {
						img(n * n_chanels + k, (n_cols * i) + j) = (loaded_data[(n_rows * n_cols * n_chanels * n)
																			   + (n_rows * k)
																			   + (n_cols * i)
																			   + j]); // 255.f - m)/s;
					}
