...
namespace simple_nn
{
	...
	public:
		...
		MatXf evaluate(const MatXf& data);
	private:
		void forward(const MatXf& X, bool is_training);
		void classify(const MatXf& output, MatXi& classified);
		void error_criterion(const VecXi& classified, const MatXi& labels, float& error_acc);
		void loss_criterion(const MatXf& output, const MatXf& labels, float& loss_acc);
		void zero_grad();
		void backward(const MatXf& X);
		void update_weight(int dc);
		int count_params();
	...
	void SimpleNN::fit(const DataLoader& train_loader, int epochs, const DataLoader& valid_loader)
	{
		...
		for (int e = 0; e < epochs; e++) {
			float loss = 0.f;
			int dc = 0;
			cout << "[Epoch:" << setw(3) << e + 1 << "/" << epochs << " ]" << endl;

			system_clock::time_point start = system_clock::now();
			for (int n = 0; n < n_batch; n++) {
				X = train_loader.get_x(n);
				Y_M = train_loader.get_y_matrix(n);

				forward(X, true);
				dc++;
				zero_grad();
				loss_criterion(net.back()->output, Y_M, loss);
				backward(X);
				update_weight(dc);
			}
			...
			if (n_batch_valid != 0) {
				for (int n = 0; n < n_batch_valid; n++) {
					...
					Y_M = train_loader.get_y_matrix(n);
					forward(X, false);
					loss_criterion(net.back()->output, Y_M, loss_valid);
				}
			}
	...
	void SimpleNN::classify(const MatXf& output, MatXi& classified)
	{
		// assume that the last layer is linear, not 2d.
//		assert(output.size() == classified.size());
	...
	void SimpleNN::error_criterion(const VecXi& classified, const MatXi& labels, float& error_acc)
	{
...
//			if (classified[i] != labels[i]) error++;
	...
	void SimpleNN::loss_criterion(const MatXf& output, const MatXf& labels, float& loss_acc)
	{
	...
	void SimpleNN::update_weight(int dc)
	{
		float lr = optim->lr();
		float decay = optim->decay();
		for (const auto& l : net) {
			l->update_weight(lr, dc);
		}
	}
	...
	MatXf SimpleNN::evaluate(const MatXf& data)
	{
		forward(data, false);
//		cout << net.back()->output << endl;
