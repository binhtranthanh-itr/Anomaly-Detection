...
from utilities.initializers import glorot_uniform, he_uniform
from utilities.settings import get_layer_num, inc_layer_num


class Convolution:
    def __init__(self, filters, kernel_shape=(3, 3), padding='same', stride=1, name=None):
        self.params = {
            'filters': filters,
            'padding': padding,
    ...
    def save_weights(self, dump_path):
        ...
        if os.path.exists(save_path):
            try:
                remove(save_path)
                remove(save_path.replace('pickle', 'json'))
            except:
                pass
        ...
        import json
        if len(list(dump_cache.keys())) > 0:
            tmp_dump_cache = dict()

        for key in dump_cache.keys():
            if len(list(dump_cache[key].keys())) > 0:
                tmp_dump_cache[key] = dict()
            for i_key in dump_cache[key].keys():
                tmp_dump_cache[key][i_key] = (dump_cache[key][i_key]).tolist()

        with open(save_path.replace('pickle', 'json'), 'w') as f:
            f.write(json.dumps(tmp_dump_cache))
    ...
    def forward_propagate(self, X, save_cache=False):
        ...
        if 'W' not in self.params:
            ...
            self.params['W'], self.params['b'] = he_uniform(shape=shape)
    ...
    def apply_grads(self, learning_rate=0.001, l2_penalty=1e-2, optimization='adam', epsilon=1e-8,
